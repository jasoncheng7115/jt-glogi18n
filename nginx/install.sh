...
# returns 0 if an existing Graylog reverse proxy is detected (not owned by us)
...
install_nginx() {
    ...
    case "$INIT" in
        systemd) run systemctl enable --now nginx 2>/dev/null \
                    || run systemctl start nginx || true ;;
        openrc)  run rc-update add nginx default 2>/dev/null || true
                 run rc-service nginx start 2>/dev/null || true ;;
        sysv)    run service nginx start 2>/dev/null || true ;;
    esac
    ok "nginx installed"
}
...
backup_file() {
    ...
    cp -a "$src" "$dest/"
    echo "$dest/$(basename "$src")" > "$BACKUP_ROOT/LATEST"
    vlog "backup: $src -> $dest"
}

rollback_latest() {
    [ -s "$BACKUP_ROOT/LATEST" ] || die "No backup available to roll back to"
    local path; path="$(cat "$BACKUP_ROOT/LATEST")"
    [ -f "$path" ] || die "Backup file missing: $path"
    local base target
    base="$(basename "$path")"
    case "$base" in
        graylog-i18n.conf)  target="$NGINX_CONF"  ;;
        *)                  die "Unknown backup type: $base" ;;
    esac
    confirm "Restore $path to $target ?" y || return 0
    cp -a "$path" "$target"
    ok "Restored $target"
    nginx -t && reload_nginx && ok "nginx reloaded"
}
...
apply_selinux_context() {
    ...
    if command -v chcon >/dev/null 2>&1; then
        run chcon -Rt httpd_sys_content_t "$INSTALL_DIR" 2>/dev/null \
            && ok "SELinux: applied httpd_sys_content_t to $INSTALL_DIR" \
            || warn "SELinux chcon failed (may need: semanage fcontext ...)"
    fi
...
test_nginx_or_restore() {
    ...
    err "nginx -t failed:"
    nginx -t 2>&1 | sed 's/^/    /'
    if [ -s "$BACKUP_ROOT/LATEST" ]; then
        local bak; bak="$(cat "$BACKUP_ROOT/LATEST")"
...
cmd_status() {
    ...
    if [ -d "$BACKUP_ROOT" ]; then
        local n; n="$(ls "$BACKUP_ROOT" 2>/dev/null | grep -v '^LATEST$' | wc -l | tr -d ' ')"
        ok "Backups: $n"
    fi
    echo
    if command -v nginx >/dev/null 2>&1; then
